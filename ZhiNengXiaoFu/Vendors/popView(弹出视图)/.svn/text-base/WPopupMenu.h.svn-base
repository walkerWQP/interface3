...
typedef NS_ENUM(NSInteger , WPopupMenuType) {
    WPopupMenuTypeDefault = 0,
    WPopupMenuTypeDark
};
...
/**
 ...
 当控件超出屏幕时会自动调整成反方向
 */
typedef NS_ENUM(NSInteger , WPopupMenuPriorityDirection) {
    WPopupMenuPriorityDirectionTop = 0,  //Default
    WPopupMenuPriorityDirectionBottom,
    WPopupMenuPriorityDirectionLeft,
    WPopupMenuPriorityDirectionRight,
    WPopupMenuPriorityDirectionNone      //不自动调整
};

@class WPopupMenu;

@protocol WPopupMenuDelegate <NSObject>
...
- (void)WPopupMenuBeganDismiss;
- (void)WPopupMenuDidDismiss;
- (void)WPopupMenuBeganShow;
- (void)WPopupMenuDidShow;

@end


@interface WPopupMenu : UIView
 ...
 */
@property (nonatomic, assign) WPopupMenuArrowDirection arrowDirection;
...
/**
 ...
 当控件超出屏幕时会自动调整箭头位置
 */
@property (nonatomic, assign) WPopupMenuPriorityDirection priorityDirection;
...
/**
 设置显示模式 Default is WPopupMenuTypeDefault
 */
@property (nonatomic, assign) WPopupMenuType type;

/**
 代理
 */
@property (nonatomic, weak) id <WPopupMenuDelegate> delegate;
...
/**
 ...
 @param delegate  代理
 */
+ (WPopupMenu *)showAtPoint:(CGPoint)point
                      titles:(NSArray *)titles
                       icons:(NSArray *)icons
                   menuWidth:(CGFloat)itemWidth
                    delegate:(id<WPopupMenuDelegate>)delegate;
...
+ (WPopupMenu *)showAtPoint:(CGPoint)point
                      titles:(NSArray *)titles
                       icons:(NSArray *)icons
                   menuWidth:(CGFloat)itemWidth
               otherSettings:(void (^) (WPopupMenu * popupMenu))otherSetting;
...
/**
 ...
 @param delegate  代理
 */
+ (WPopupMenu *)showRelyOnView:(UIView *)view
                         titles:(NSArray *)titles
                          icons:(NSArray *)icons
                      menuWidth:(CGFloat)itemWidth
                       delegate:(id<WPopupMenuDelegate>)delegate;
