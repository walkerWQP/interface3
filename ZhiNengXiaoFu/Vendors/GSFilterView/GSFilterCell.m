...
- (instancetype)init{
    if (self = [super init]) {
        ...
        //颜色的设置
        self.buttonNormalColor = RGB(51,51,51);   //未选中标题颜色
        self.buttonHighlightColor = RGB(0,172,241); //选中标题颜色
        self.buttonNormalBorderColor = [UIColor clearColor]; //未选中边框颜色
        self.buttonSelectBorderColor = RGB(0,172,241); //选中边框颜色
        self.buttonSelectBackGroundColor = [UIColor whiteColor]; //选中按钮的背景色
        self.buttonNormalBackGroundColor = RGB(239, 239, 244);//未选中按钮背景颜色
...
- (void)initButtons{ 
    ...
    Btnx = (APP_WIDTH - 70 * 3) / 3.5;//每个btn的起始位置
    ...
    [_filterData.elements enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if(Btnx + btnWidth > oneLineBtnWidtnLimit)
        {
            ...
            Btnx = (APP_WIDTH - 70 * 3) / 3.5 + btnGap;
        }
        ...
        button.backgroundColor = self.buttonNormalBackGroundColor;
        button.selected = NO;
        button.tag = idx;
