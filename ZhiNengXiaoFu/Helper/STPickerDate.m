...
static NSInteger const yearSum = 4;
...
- (void)loadData
{
    ...
    _currentDay = _day;
     ...
     self.toolbar.title = [NSString stringWithFormat:@"%ld年%ld月%ld日", self.year, self.month, self.day];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    NSInteger yearSelected = [pickerView selectedRowInComponent:0] + _currentYear;
    NSInteger monthSelected;
    if (component == 0) {
        return yearSum;
    }else if(component == 1) {
        if (yearSelected == _currentYear) return(12 - _currentMonth +1);
           return 12;
    }else {
        
        if (yearSelected == _currentYear)
          monthSelected    = [pickerView selectedRowInComponent:1] + _currentMonth;
        else
         monthSelected    = [pickerView selectedRowInComponent:1] +1;

        if (yearSelected == _currentYear && monthSelected == _currentMonth) {
            
          return ([NSCalendar getDaysWithYear:yearSelected month:monthSelected] -_currentDay + 1);
        }
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    NSInteger monthSelected;
    if (component == 0) {
        text =  [NSString stringWithFormat:@"%ld", row + _currentYear];
    }else if (component == 1){
        if (yearSelected ==_currentYear ) {
            text =  [NSString stringWithFormat:@"%ld", row + _currentMonth];
        ...
        if (yearSelected == _currentYear && monthSelected == _currentMonth) {
            ...
            text = [NSString stringWithFormat:@"%ld", row + _currentDay];
        }else{
            text = [NSString stringWithFormat:@"%ld", row + 1];
        }
...
- (void)reloadData
{
    self.year  = [self.pickerView selectedRowInComponent:0] + _currentYear;
    if (self.year == _currentYear ) {
        self.month = [self.pickerView selectedRowInComponent:1] +_currentMonth;
    }else{
        self.month = [self.pickerView selectedRowInComponent:1] + 1;
    }

    if (self.year == _currentYear && self.month == _currentMonth) {
       self.day   = [self.pickerView selectedRowInComponent:2] + _currentDay;
    }else{
